...
$res = mysqli_query($con,"select * from coupon_master where coupon_code = '$coupon_str'");
...
if($count > 0){
    ...
    if($cart_min_value>$cart_total){
        $jsonArr = array('is_error'=>'yes','result'=>$cart_total,'dd'=>'Cart total amount must be '.$cart_min_value);
    }else{
